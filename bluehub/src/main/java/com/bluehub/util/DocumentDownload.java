...
public class DocumentDownload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		UserProfile user = (UserProfile) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		 ...
		 String destinationPath=Constants.getPropertyValue(Constants.FAREFILE_UPLOAD_PATH);
		...
		try {
			...
			saveDirectory = destinationPath  + userId +  syntax + fileName;
			/*if(visitid==null){
				saveDirectory = destinationPath  + userId +  syntax + fileName;
			}else{
				saveDirectory = destinationPath  + userId +  syntax + visitid + syntax + fileName;
			}*/
			
			logger.debug("DocumentDownload saveDirectory==>" + saveDirectory);
