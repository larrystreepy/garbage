	...
	function submitTag(){
		...
		if(note==""){
			...
			$.ajax({
				...
				url : contextPath + "/administrator/patientnoteentry.do",
				data : "note=" + note +"&visitid="+visitid+"&method=note&type="+type,
...
// 				data : "note=" + note +"&noteid="+noteid+"&method=note",
				data : "note=" + note +"&noteid="+noteid+"&method=note&type="+type,
				 /* data : "note=" + note +"&visitid="+visitid, */
				cache : false,
				async : false,
				success : function(response) {
	...
	function fnSetVisitvalue1(id){
		...
		$.ajax({
			...
			data : "visitid="+visitid+"&method=note&type=Document",
			cache : false,
			async : false,
			success : function(response) {
	...
	function fnSetVisitvalue(id,patid){
		...
		$.ajax({
			type : "GET",
			url : contextPath + "/administrator/checknoteentry.do",
			data : "visitid="+visitid+"&type=Visit&method=note",
			cache : false,
			async : false,
			success : function(response) {
					...
					success : function(response) {
						if(response=="null" || response==""){
							...
							for (var i = 0; i < parsedJson.length; i++) {
								...
								/* content += '<td ><input type="radio" id="visitRadio" value='+parsedJson[i].visitid+' name="visitRadio"></td>'; */
...
<div class="content-body">
	...
	<div id="panel-typeahead"
		...
		<div class="panel-heading">
			...
			<h3 class="panel-title">Private Note</h3>
		</div>
