...
function searchPhysicianVisitRecords1(physicianid){
	displayPhysicianUserData(physicianid);
				
}

function displayPhysicianUserData(visitid){
	var patid = 0;
		$.ajax({
			type : "GET",
			url : contextPath + "/administrator/adminphysiciansearchdetailsvisit.do",
			cache : false,
...
function searchPhysicianVisitRecords(physicianid){
	searchPhysicianVisitRecords1(physicianid);
	// document.getElementById('visitRecords').style.display="block";
